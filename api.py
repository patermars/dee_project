...
# TOMTOM_API_KEY = "9xtqmSW9YaemaETeMYOVMA0ZDnbukU8i"
TOMTOM_API_KEY = "UGueLwdYpWdFsN9O8q1PW3Mu3bSIAuTD"
CALL_INTERVAL_SECONDS = 30
DATA_RETENTION_MINUTES = 10  # Only keep data for 10 minutes
...
# Expanded traffic flow points for better map visualization
TRAFFIC_FLOW_POINTS = [
...
def create_tables():
    ...
    # Create traffic_flow table with unique constraint on location_name
    cursor.execute("""
    ...
    # Create traffic_incidents table with unique constraint on incident_id
    cursor.execute("""
    ...
    # Create route_health table with unique constraint on route_name
    cursor.execute("""
    ...
    # Create hourly_aggregates table with unique constraint on route_name and hour_of_day
    cursor.execute("""
    ...
    # Create vehicle_classification table with unique constraint on location_name and vehicle_type
    cursor.execute("""
    ...
    # Modified speed_distribution table with only relevant speed ranges
    cursor.execute("""
    ...
    # Create congestion_hotspots table with unique constraint on location_name
    cursor.execute("""
    ...
    # Create daily_kpi table with unique constraint on date
    cursor.execute("""
...
def fetch_traffic_flow(lat, lon, zoom=15):
    ...
    try:
        ...
        print(f"[ERROR] Traffic flow API call failed: {e}")
        return None
...
def parse_traffic_flow(data, location_name):
    try:
        ...
        # Add some variation to the speed data
        current_speed = flow_data.get("currentSpeed", 0)
        if current_speed > 0:
            # Add random variation between -10% and +10%
            variation = random.uniform(0.9, 1.1)
            current_speed = max(5, current_speed * variation)  # Ensure minimum speed of 5
        ...
        print(f"[ERROR] Parsing traffic flow failed: {e}")
        return None
...
def fetch_incidents(bbox):
    ...
    try:
        ...
        print(f"[ERROR] Incident API call failed: {e}")
        return None
...
def parse_incidents(data):
    ...
    try:
        ...
        print(f"[ERROR] Parsing incidents failed: {e}")
...
def convert_iso_to_mysql_datetime(iso_string):
    ...
    try:
        ...
        dt = datetime.datetime.strptime(iso_string.replace('Z', '+00:00').split('+')[0], '%Y-%m-%dT%H:%M:%S')
...
def calculate_vehicle_distribution(flow_points_data):
    # Add variation to vehicle distribution
    base_distribution = {
        "Cars": 0.65,
        "Trucks": 0.15,
        "Buses": 0.10,
    ...
    # Add random variation to percentages
...
def calculate_speed_distribution(flow_points_data):
    # Only include relevant speed ranges (0-20 and 20-40)
    speed_ranges = [
    ...
    for speed_range in speed_ranges:
        ...
        # Add variation to make the data more realistic
        if count > 0:
...
def identify_congestion_hotspots(flow_points_data):
    ...
    for point in flow_points_data:
        if point.get("current_speed", 0) > 0 and point.get("free_flow_speed", 0) > 0:
            ...
            # Add variation to congestion score
            congestion_score *= random.uniform(0.9, 1.1)
            congestion_score = min(100, congestion_score)  # Cap at 100
...
def calculate_daily_kpi(flow_points_data, incidents_count):
    ...
    # Add variation to vehicle count
    ...
    avg_speed *= random.uniform(0.95, 1.05)  # Add 5% variation
    ...
    avg_travel_time *= random.uniform(0.95, 1.05)  # Add 5% variation
    
    # Calculate delays with variation
    delays = [point.get("current_travel_time", 0) - point.get("free_flow_travel_time", 0) for point in flow_points_data]
    avg_delay = sum(delays) / len(delays) if delays else 0
    avg_delay *= random.uniform(0.9, 1.1)  # Add 10% variation
    ...
    # Add variation to change percent
...
def calculate_hourly_aggregates(route_name, flow_point_data):
    ...
    # Use actual flow data instead of constant values
    ...
    current_speed *= random.uniform(0.95, 1.05)  # 5% variation
    travel_time *= random.uniform(0.95, 1.05)  # 5% variation
    ...
    return {
        ...
        "avg_delay": delay / 60,  # Convert to minutes
        "vehicle_count": vehicle_count,
        "congestion_level": congestion_level
    }

def calculate_route_health_score(flow_point_data, incident_count):
    # Use actual flow data to calculate health score
    current_speed = flow_point_data.get("current_speed", 45)
    ...
    # Calculate delay percentage (max penalty: 40 points)
    if current_travel_time > 0 and free_flow_travel_time > 0:
        ...
        base_score -= min(delay_percent * 0.4, 40)  # Cap at 40 points penalty
    
    # Factor in speed ratio (max penalty: 30 points)
    if current_speed > 0 and free_flow_speed > 0:
        speed_ratio = current_speed / free_flow_speed
        if speed_ratio < 1:
            base_score -= (1 - speed_ratio) * 30
    
    # Factor in incidents (max penalty: 20 points, 5 points per incident)
...
def save_traffic_flow(flow_data):
    try:
        ...
        print(f"[ERROR] Saving traffic flow failed: {e}")
...
def save_incident(incident_data):
    try:
        ...
        print(f"[ERROR] Saving incident failed: {e}")
...
def save_route_health(location_name, health_score, congestion_level, delay_percent, incident_count, recommendation):
    try:
        ...
        print(f"[ERROR] Saving route health failed: {e}")
...
def save_hourly_aggregates(hourly_data):
    try:
        ...
        print(f"[ERROR] Saving hourly aggregates failed: {e}")
...
def save_vehicle_classification(location_name, distribution):
    try:
        ...
        print(f"[ERROR] Saving vehicle classification failed: {e}")
...
def save_speed_distribution(location_name, distribution):
    try:
        ...
        print(f"[ERROR] Saving speed distribution failed: {e}")
...
def save_congestion_hotspots(hotspots):
    try:
        ...
        print(f"[ERROR] Saving congestion hotspots failed: {e}")
...
def save_daily_kpi(kpi_data):
    try:
        ...
        print(f"[ERROR] Saving daily KPI failed: {e}")

def cleanup_old_data():
    """Remove data older than DATA_RETENTION_MINUTES to keep only real-time data"""
    try:
        conn = mysql.connector.connect(**DB_CONFIG)
        cursor = conn.cursor()
        
        # Calculate cutoff time
        cutoff_time = datetime.datetime.utcnow() - datetime.timedelta(minutes=DATA_RETENTION_MINUTES)
        ...
        for table in tables:
            if table == "traffic_incidents":
                # For incidents, only remove resolved ones
                cursor.execute(f"""
                    DELETE FROM {table} 
...
def run_comprehensive_pipeline():
    print("\n" + "="*60)
    print("🚀 SMART CITY ANALYTICS - Real-Time Data Collection")
    print("="*60)
    ...
    if incident_data:
        ...
        for incident in incidents[:5]:
            ...
            print(f"   ⚠️  {incident['incident_type']}: {incident['description'][:50]}...")
    ...
    for point in flow_points_collected:
        ...
        emoji = "🟢" if health_score >= 80 else "🟡" if health_score >= 60 else "🟠" if health_score >= 40 else "🔴"
        print(f"{emoji} {point['location_name']}: Score {health_score}/100 - {congestion}")
    ...
    if flow_points_collected:
        # Save vehicle classification and speed distribution for each location
        for point in flow_points_collected:
    ...
    # Save hourly aggregates for each flow point
    for point in flow_points_collected:
...
if __name__ == "__main__":
    print("\n" + "🎯"*30)
    print("SMART CITY ANALYTICS - REAL-TIME TRAFFIC MONITORING SYSTEM")
    print("🎯"*30 + "\n")
    
    print("Initializing database tables...")
    create_tables()
    
    print(f"\n⏰ Data collection interval: {CALL_INTERVAL_SECONDS} seconds")
    print(f"🌊 Tracking {len(TRAFFIC_FLOW_POINTS)} traffic flow points")
    print(f"🚨 Incident detection: Active")
    print(f"🗑️ Data retention: {DATA_RETENTION_MINUTES} minutes\n")
    ...
    try:
        while True:
            ...
            print(f"\n⏳ Next collection in {CALL_INTERVAL_SECONDS} seconds...")
